...
Set the required environment variables shown below in a file named `.env` in the ```FinancialQA/``` directory, so that it can be discovered and loaded by the `dotenv` package at runtime. PDF files can be stored either in an Azure Blob Storage container (as configured by the ```AZURE_STORAGE_*``` variables) or locally in a `data/` folder under the `src/` directory. The ```AZURE_AI_SEARCH_INDEX_NAME``` variable can be set to the name of an existing index with the required schema, otherwise a new index with the specified name is created.
...
**Notes:**
- PDF files can be stored locally under `src/data/` or in Azure Blob Storage.
- For financial report PDFs, use the naming convention `<company_name>_<company_quarter>` (e.g., `CIBC_Q4`) to ensure correct metadata extraction and filtering in the RAG pipeline.
...
Or it can be run as a module directly from the command line (type ```--help``` after the command to see the full list of options):
